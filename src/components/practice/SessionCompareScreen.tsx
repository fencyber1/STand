...
export default function SessionCompareScreen() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{t('Compare Sessions')}</h1>
        <p className="text-gray-500 dark:text-gray-400">{t('Pick two sessions to compare side by side')}</p>
      </div>

      {history.length < 2 ? (
        <div className="text-center py-16">
          <ArrowRightLeft size={40} className="mx-auto text-gray-300 dark:text-gray-600 mb-3" />
          <p className="text-gray-500 dark:text-gray-400">{t('Need at least 2 sessions to compare')}</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{t('Session A')}</label>
              <select
                value={leftId}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{t('Session B')}</label>
              <select
                value={rightId}
            ...
            >
              <div className="p-6 dark:bg-gray-800">
                ...
                <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-1">{t('Verdict')}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {(left.score ?? 0) > (right.score ?? 0)
                      ? `${t('Session A')} (${left.topic}) ${t('scored')} ${diff(left.score ?? 0, right.score ?? 0)}% ${t('higher')}.`
                      : (left.score ?? 0) < (right.score ?? 0)
                      ? `${t('Session B')} (${right.topic}) ${t('scored')} ${diff(right.score ?? 0, left.score ?? 0)}% ${t('higher')}.`
                      : t('Both sessions scored equally!')}
                  </p>
                </div>
              </div>
            </BorderGlow>
